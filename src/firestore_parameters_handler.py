...
class FirestoreParametersHandler(FirestoreClientHandler):
    ...
    def stop(self):
        ...
        try:
            self.client.collection(self.devices_path).document(
                self.device_id).update({"connected": False}, timeout=2)
    ...
    def on_client_initialized(self):
        ...
        self.client.collection(self.devices_path).document(self.device_id).update(
            {"last_connection": int(int(datetime.now().timestamp()))}, timeout=2)
        self.client.collection(self.devices_path).document(
            self.device_id).update({"connected": True}, timeout=2)
        if self.device_subscription:
            return
    ...
    def upload_new_config(self, config):
        ...
        if not self.client:
            logger.debug(
                "Firebase client not ready, cannot upload new configuration")
            return
        updated_field = {f"{self.KEYWORD}": config}
        try:
            self.client.collection(self.devices_path).document(
    ...
    def create_device(self):
        ...
        if not self.client:
            ...
            return
        initial_data = {"creation_date": datetime.now().strftime(
            "%Y/%m/%dT%H:%M:%S"), "last_connection": 0, self.KEYWORD: {}}
        try:
            self.client.collection(self.devices_path).add(
                initial_data, self.device_id, timeout=2)
        except Exception as e:
            logger.error("Firebase client error, restart the handler")
            threading.Timer(self.RESTART_DELAY_TIME_S, self.restart).start()
    ...
    def add_parameters(self, parameters_value, parameters_key):
        ...
        if not self.client:
            logger.debug(
                "Firebase client not ready, add new parameters configuration")
            return
    ...
    def _on_device_update(self, document_snapshot, changes, read_time):
        ...
        cloud_config = self._filter_last_configuration(device_info)

        threading.Thread(target=self.parameters_update_callback,
                         args=[cloud_config]).start()

    def _filter_last_configuration(self, device_info):
        device_configuration = device_info.get(self.KEYWORD, {})

...
def stop():
    ...
    if firestore_parameters_handler:
        firestore_parameters_handler.stop()
    if comm_module:
        comm_module.stop()
...
def on_firestore_parameters_update(firestore_config):
    ...
    if not firestore_config:
        logger.warning(
            "No configuration available in the cloud, uploading the local one")
        firestore_parameters_handler.upload_new_config(local_config)
        return
    ...
    if firestore_config_date == local_config_date:
        logger.info(
            f"Both configs are equal, nothing has changed ({local_config_date})")
    elif firestore_config_date > local_config_date:
        logger.info(
            "There is a new configuration in the cloud, saving it locally")
...
if __name__ == "__main__":
    ...
    firestore_parameters_handler = FirestoreParametersHandler(
        api_key, project_id, refresh_token, device_id, on_firestore_parameters_update)
